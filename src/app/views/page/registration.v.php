<form method="post" id="register_form" class="form-horizontal" >
	...
	<div class="form-group row" id="email_form_group">
		...
		<div class="form-validation" id="email_validation_exists"><?=$this->t('This email is already used!') ?></div>
	</div>

	<div class="form-group row">
		<label for="phone" class="col-sm-2 control-label"><?=$this->t('Phone') ?>:</label>
		<div class="col-sm-4">
			<input type="text" id="phone" name="phone" class="form-control" value="<?=$this->xssafe(z::get('phone')) ?>" />
		</div>
	</div>

	<div class="form-group row" id="password_form_group">
		<label for="password" class="col-sm-2 control-label"><?=$this->t('Heslo') ?>:</label>
		<div class="col-sm-4">
			<input type="password" id="password" name="password" class="form-control" required />
		</div>
		<div class="form-validation" id="password_validation_password"><?=$this->t('Password must be at least %d characters long.', $this->z->auth->getConfigValue('min_password_length')) ?></div>
	</div>

	<div class="form-group row" id="password_confirm_form_group">
