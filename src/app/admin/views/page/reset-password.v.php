	...
	if ($this->getData('show_form')) {
		?>
			<form id="password_reset_form" method="POST" >
				...
				<input type="hidden" name="user_email" value="<?=$this->getData('user_email') ?>" />
				...
				<div class="form-group row">
					<div class="col-sm-3 offset-sm-2">
						<input type="submit" onclick="javascript:validateResetForm();return false;" class="btn btn-success" value="<?=$this->t('Reset Password') ?>">
					</div>
				</div>
			</form>
		<?php
		...
		?>
			<a href="<?=$this->url('admin') ?>"><?=$this->t('Sign In') ?></a>
		<?php
	}
?>
