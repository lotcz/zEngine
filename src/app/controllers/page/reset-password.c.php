<?php
	...
	if (isset($reset_token) && isset($customer_email)) {
		...
		$customer->loadByLoginOrEmail($customer_email);

		$token_not_expired = ($customer->val('user_reset_password_expires') > z::mysqlTimestamp(time()));
		$token_valid = $this->z->auth->verifyPassword($reset_token, $customer->val('user_reset_password_hash'));
		...
		if ($customer->is_loaded && $token_not_expired && $token_valid) {
			...
			if (isset($password) && isset($password2)) {
				if ($password == $password2) {
					$customer->set('user_state', UserModel::user_state_active);
					$customer->set('user_password_hash', $this->z->auth->hashPassword($password));
					$customer->set('user_reset_password_hash', null);
					$customer->set('user_reset_password_expires', null);
					$customer->save();
