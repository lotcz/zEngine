...
class globalsModule extends zModule {
	...
	function onEnabled() {
		$this->z->db = $this->z->db->connection;
		$stmt = SqlQuery::select($this->z->db, 'site_globals');
	...
	function processForm($form) {
		...
		foreach ($values as $key => $value) {
			SqlQuery::update($this->z->db, 'site_globals', ['site_global_value' => $value], 'site_global_name = ?', [$key]);
			$this[$key] = $value;
		}
